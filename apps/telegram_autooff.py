...
#
# App to request to turn off stuff left powered on if someone leaves home
#
...
class TelegramAutoOff(hass.Hass):
  ...
  def receive_telegram_callback(self, event_id, payload_event, *args):
      ...
      params = data_callback.replace("/","").split(";")
      if data_callback == "/autooff;" + self.args["instance_name"] + ";turn_off_all":
      ...
      if old == "home" and new == "not_home":
          if self.all_not_home():
              if self.something_on():
                  ...
                  user_cached = self.get_user_by_tracked_device(entity)
                  if user_cached is None:
                      self.log("User for device " + str(entity) + " was not found!")
                      return
                  service_name = str(user_cached["notify"]).replace(".", "/")
                  online = []
                  ...
                  keyboard = ["Ausschalten:/autooff;" + self.args["instance_name"] + ";turn_off_all"]
